...
## Principle 9: Validate Parity Dynamically Against a Real-World Corpus

**Static assertions (hardcoded expected values) are necessary but insufficient.
Parity must also be validated dynamically by running both implementations against a
...
1. **Maintain a corpus of real-world inputs** drawn from production data, edge cases
   discovered during porting, and representative documents across all supported formats.
...
- [ ] Corner-cases discovered through corpus validation have been extracted into
  permanent regression test fixtures.
...
| 9 | Validate parity dynamically against a corpus | False parity through wrong expected values |
