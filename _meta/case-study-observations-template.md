...
`case-studies/<project-name>/<project>-port-observations.md` before starting the port.
Fill in each phase section as you complete it.
Do not skip phases — even if a phase was trivial, record that it was trivial and why.

**Observation IDs:** Number each observation sequentially across all phases (OBS-1,
OBS-2, etc.) so they can be referenced during triage.
A phase may produce zero or multiple observations.
